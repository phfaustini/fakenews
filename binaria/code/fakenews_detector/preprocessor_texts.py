...
class PreProcessorTexts(PreProcessor):
    ...
    def _get_label(self, label: str, platform="Websites") -> float:
        """In general, true news are labelled as 1.0,
        and fake news are 0.0. However, in sklearn OCC,
        target class (here, fake news) are 1.0, and ouliers
        (here, true news), are -1.0.
        This method convert a string label to its correct
        float representation.
        :param label: either 'True' or 'False'
        :param platform: either 'Websites' or 'WhatsApp'
        :return: 1.0 or 0.0 (Websites) or 1.0 or -1.0 (WhatsApp)
        """
        if platform == 'Websites':
            return 0.0 if label == "False" else 1.0
        else:
            return 1.0 if label == "False" else -1.0
        ...
        return pd.DataFrame(
                            [
                              [
                                ...
                                obj.word2vec,
                                obj.text.replace('\n', ' '),
                                self._get_label(label=label, platform=platform)
                              ]
                            ],

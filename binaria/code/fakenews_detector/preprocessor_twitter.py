...
class PreProcessorTwitter(PreProcessor):
    ...
    def __init__(self, language: str, dataset: str, embeddings_path: str):
        """
        :param dataset: 'tweets_br'
        """
        PreProcessor.__init__(self, language=language, embeddings_path=embeddings_path, platform_folder="datasets/Twitter/{0}/".format(dataset))
    ...
    def _convert_json_to_obj(self, json: dict) -> Tweet:
        try:
            ...
            obj.full_text.NOUN = self.get_proportion_tag(json['full_text'], 'NOUN')
            ...
            obj.full_text.word2vec = self.get_word2vec_mean(obj.full_text.text)
    ...
    def convert_obj_to_dataframe(self, label: str, obj: Tweet, platform: str) -> pd.DataFrame():
        """
        ...
        return pd.DataFrame(
                            [
                              [
                                ...
                                obj.full_text.word2vec,
                                obj.full_text.text.replace('\n', ' '),
                                0.0 if label == "False" else 1.0
                              ]
                            ],
                            columns=["uppercase", "exclamation", "has_exclamation", "question", "words_per_sentence", "adj", "adv", "noun", "spell_errors", "lexical_size", "polarity", "number_sentences", "len_text", "word2vec", "Text", "label"]
                           )
